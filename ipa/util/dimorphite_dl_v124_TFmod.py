...
#
# This particular version of Dimorphite-DL (ver. 1.2.4) was modified by
# Dr. Tony Froehlich to be used with Protoro, which is an extension of
# Dimorphite-DL to enable protonation state generation of 3D structures
# (.sdf and .pdb files). Dimorphite-DL is limited to SMILES strings.

"""
This script identifies and enumerates the possible protonation sites of SMILES
strings. The protoro extension enables the protoantion state generation of 3D
structures (.sdf and .pdb files).
"""
...
def main(params=None):
    ...
    params["input"] = None
    params["smiles_file"] = None
    params["output_file"] = None
    params["max_variants"] = 64
    params["label_states"] = False
    params["silent"] = False
    params["test"] = False
    return list(Protonate(params))
...
class TestFuncs:
    ...
    def test():
        ...
        output = list(Protonate({"smiles": smi, "test": False, "silent": True}))
        if len(output) != 64:
            msg = "Processing " + smi + " produced more than 64 variants!"
            raise Exception(msg)
        else:
            print("(CORRECT) Produced 64 variants: " + smi)
...
def run_with_mol_list(mol_lst, **kwargs):
    ...
    # that a list of Mol objects can be used directly. Instead, convert this
